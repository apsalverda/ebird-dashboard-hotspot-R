...
#    embed-resources: true
    favicon: favicon.ico
    orientation: columns
    loading: "Loading eBird user dashboard ..."
    expandable: false
    theme: 
      - cosmo
      - styles.css
...
if (!file.exists("MyEBirdData.csv")){
  stop("Cannot compile document. Missing required file \"MyEBirdData.csv\".\nPlease add this file with your personal eBird data to the folder from which you compile the dashboard")
}
...
# check if event logging is possible
folder = "/var/log/ebird-dashboard-hotspot-R"
file = file.path(folder, "sessions.log")
log_events = TRUE
tryCatch(
  if (!dir.exists(folder)) dir.create(folder, recursive = TRUE),
  warning = function(w) log_events <<- FALSE,
  error = function(e) log_events <<- FALSE
)
tryCatch(
...
# check if temporarily saving data is possible
folder = "/tmp/ebird-dashboard-hotspot-R"
file = file.path(folder, paste0(sample(letters), collapse = ""))
temp_store_data = TRUE
tryCatch(
  if (!dir.exists(folder)) dir.create(folder, recursive = TRUE),
  warning = function(w) temp_store_data <<- FALSE,
  error = function(e) temp_store_data <<- FALSE
)
tryCatch(
  if (!file.exists(file)) file.create(file),
  warning = function(w) temp_store_data <<- FALSE,
...
filters  <- reactiveValues(country = "All", state = "All", location = "All")
previous <- reactiveValues(country = "All", state = "All", location = "All")
...
observeEvent(input$upload, {
  ...
  if (temp_store_data){
    filename = paste0("/tmp/", session$token, ".rds")
    write_rds(imported_dat, filename, compress = "gz")
  }
})
...
selected_dat = reactive({
  ...
  if (filters$state != "All"){
    # user has selected a state
    my_selected_dat =
      my_selected_dat %>%
      filter(state_province == filters$state)
  }
  if (!identical(min(my_selected_dat$date), isolate(earliest_date()))) earliest_date(min(my_selected_dat$date))
  if (!identical(max(my_selected_dat$date), isolate(latest_date()))) latest_date(max(my_selected_dat$date))
  ...
  updateSelectInput(
    session,
    "hotspot",
    choices = sort_locations(my_selected_dat_for_sorting, isolate(input$hotspot_sorting)),
    selected = isolate(input$hotspot)
  )
  ...
  if (!identical(min(as.numeric(as.character(my_selected_dat$year))), isolate(earliest_year()))) earliest_year(min(as.numeric(as.character(my_selected_dat$year))))
  if (!identical(max(as.numeric(as.character(my_selected_dat$year))), isolate(latest_year()))) latest_year(max(as.numeric(as.character(my_selected_dat$year))))
  top_lat_lng =
    my_selected_dat %>%
  ...
  if (!identical(top_lat_lng$latitude,  isolate(hotspot_latitude()))) hotspot_latitude(top_lat_lng$latitude)
  if (!identical(top_lat_lng$longitude, isolate(hotspot_longitude()))) hotspot_longitude(top_lat_lng$longitude)
  if (!identical(top_lat_lng$location,  isolate(hotspot_name()))) hotspot_name(top_lat_lng$location)
  ...
  girafe(
    ...
    girafe_options(
      ...
      opts_toolbar(
        ...
        hidden = c("selection", "saveaspng", "zoom", "fullscreen")
      ),
      opts_tooltip(
        use_fill = TRUE,
        opacity = .7,
...
observeEvent(c(input$date_range_slider, input$ebird_dat, input$selected_dat), {
  if (difftime(Sys.time(), change_slider()) > react_delay) {
    new_start <- input$date_range_slider[[1]]
    new_end   <- input$date_range_slider[[2]]
    if (!identical(new_start, isolate(input$date_range[[1]])) || 
        !identical(new_end,   isolate(input$date_range[[2]]))) {
...
observe(
  {
    ...
    current_location = input$hotspot
    
    previous_country = isolate(previous$country)
    previous_state = isolate(previous$state)
    previous_location = isolate(previous$location)
    
    country_changed = !identical(current_country, previous_country)
    state_changed = !identical(current_state, previous_state)
    location_changed = !identical(current_location, previous_location)
    
    if (country_changed){
      # user changed country
      # -> if user selects a specific country, show only the state/province
      #    and hotspot values associsted with that country
      filters$country = current_country
      filters$state = "All"
      filters$location = "All"
      filtered_dat = 
        if(current_country == "All") isolate(ebird_dat()) else isolate(ebird_dat()) |> filter(country == current_country)
      states = c("All", sort(unique(filtered_dat$state_province)))
      locations = c("All", sort(unique(filtered_dat$location)))
      updateSelectInput(
        session, 
        "state",
        choices = states,    
      ...
      updateSelectInput(
        session, 
        "hotspot",
        choices = sort_locations(filtered_dat, isolate(input$hotspot_sorting)),
        selected = "All"
      )
      previous$country = current_country
      previous$state = "All"
      previous$location = "All"
    } else if (state_changed){
      # user changed state
      # -> if user selects a specific state/province, change country filter and
      #    hotspot filters to show only values associated with that state/province
      if (current_state == "All"){
        ...
        updateSelectInput(
          session,
          "state",
          choices = c("All", filtered_dat |> distinct(province_state) |> arrange(province_state) |> pull()),
          select = "All"
        )
        updateSelectInput(
          session,
        ...
        filtered_dat = isolate(ebird_dat()) |> filter(state_province == current_state)
        parent_country = unique(filtered_dat$country)
        filters$country = parent_country
        filters$state = current_state
        filters$location = "All"
        updateSelectInput(
          session, 
          "country", 
          selected = parent_country
        )
        updateSelectInput(
          session,
          "state",
          choices = c("All", isolate(ebird_dat()) |> filter(country == parent_country) |> distinct(state_province) |> arrange(state_province) |> pull()),
        ...
        previous$country = parent_country
        previous$state = current_state
        previous$location = "All"
      }
    } else if (location_changed){
      if (is.null(current_location) || current_location == "") 
      {
        previous$location = ""
        return()
      }
      ...
      filtered_dat = isolate(ebird_dat())
      if (filters$country != "All") filtered_dat = filtered_dat |> filter(country == filters$country)
      if (filters$state != "All") filtered_dat = filtered_dat |> filter(state_province == filters$state)
      updateSelectInput(
        session,
        "hotspot",
        choices = sort_locations(filtered_dat, isolate(input$hotspot_sorting)),
...
observeEvent(
  input$hotspot_sorting,
  {
    updateSelectInput(
      session, 
      "hotspot",
      choices = sort_locations(isolate(selected_dat()), isolate(input$hotspot_sorting)),  
      selected = isolate(input$hotspot)
    )
  }
)
...
observeEvent(
  # runs on initialization, or when new data is imported
  c(session$clientData$url_hostname, input$upload),
  { 
    country_list =
      ebird_dat() %>%
      distinct(country) %>%
    ...
    #    reactive_country_values(country_list)
    state_list =
      ebird_dat() |> 
      distinct(state_province) |> 
      pull() |> 
      sort()
    state_list = c("All", state_list)
    updateSelectInput(
      session, 
      "state",
      choices = state_list,  
    ...
    #    reactive_state_values(state_list)
    updateSelectInput(
      session, 
      "hotspot",
      choices = sort_locations(ebird_dat(), isolate(input$hotspot_sorting)),  
      selected = "All"
    )

...
selected_dat = reactive({
  ...
  ebird_observation_types = paste0("eBird - ", input$observation_types)
  my_selected_dat = my_selected_dat %>%
    filter(
      date >= input$date_range[1],
      date <= input$date_range[2]
    ) %>%
#    filter(protocol %in% c("eBird - Traveling Count", "eBird - Stationary Count", "eBird - Casual Observation"))
#    filter(protocol == "eBird - Traveling Count")
...
output$species_count_plot = renderGirafe({
  ...
  # median counts by season by year
  median_counts_by_season = 
    selected_dat() %>% 
    count(submission_id, location, date, us_season) |> 
    mutate(year = year(date)) |>
#    group_by(year, us_season) |> 
#    mutate(median_count = median(n)) |> 
#    ungroup() |> 
    select(year, us_season, date, n) |> 
    full_join(
      tibble(
        date = seq.Date(min(selected_dat()$date), max(selected_dat()$date), by = "day")),
      by = "date"
    ) |> arrange(date) |> 
    mutate(
      year = year(date),
    ...
    mutate(
      color = case_when(
        input$season_coloring == "None" ~ "#707070",
        TRUE ~ "#000000"
      )
    )
